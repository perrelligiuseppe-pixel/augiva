...
export async function POST(request) {
  try {
    ...
    if (compErr || !company) return NextResponse.json({ error: 'Profilo azienda non trovato' }, { status: 404 })
    ...
    const { data: job, error: jobErr } = await supabase
      .from('precompile_jobs').insert({ company_id, tender_id, status: 'pending', progress_step: 'In attesa di avvio...' })
      .select('id').single()
    if (jobErr) throw jobErr

    const job_id = job.id
    const child = spawn('python3', ['/root/augiva-backend/precompiler.py', job_id, company_id, tender_id], {
